...
// ==========================================
// 🛠️ ADMIN TOOL: INJECT 34 DATABASE TEAMS FOR TESTING
// ==========================================
function injectDatabaseDummyButton() {
    // Only show to Admin (optional, but good practice so real users don't see it)
    const ADMIN_EMAIL = "efootballleague369@gmail.com";
    if (!currentUser || currentUser.email !== ADMIN_EMAIL) return;

    // Check if the button already exists
    if (document.getElementById('dummyBtn')) return;

    const dummyBtn = document.createElement('button');
    dummyBtn.id = 'dummyBtn';
    dummyBtn.innerText = "🛠️ Inject 34 Dummy Teams (Database)";
    dummyBtn.style.cssText = "position:fixed; bottom:80px; right:5px; z-index:9999; padding:15px; background:#16a34a; color:white; border:none; border-radius:8px; cursor:pointer; font-weight:bold; box-shadow: 0 4px 6px rgba(0,0,0,0.3);";
    
    dummyBtn.onclick = async function() {
        if (!confirm("This will write 34 new teams to your Firestore database. Proceed?")) return;
        
        dummyBtn.innerText = "⏳ Injecting... Please wait";
        dummyBtn.disabled = true;

        try {
            const batch = writeBatch(db);
            
            // Loop 34 times instead of 36
            for (let i = 1; i <= 34; i++) {
                // Generate a new unique document reference
                const newTeamRef = doc(collection(db, "league_players"));
                ...
                batch.set(newTeamRef, {
                    ...
                    name: "Dummy FC " + i,
                    teamName: "Dummy FC " + i,
                    playerName: "Manager " + i,
                    teamBadge: "https://cdn-icons-png.flaticon.com/512/5323/5323443.png",
                    status: "approved",
            ...
            await batch.commit();
            alert("✅ 34 Teams successfully written to Firestore! Now add your 2 real users.");
            
            dummyBtn.remove(); // Remove button to prevent accidental double-clicks
            
        } catch (error) {
            console.error("Error injecting teams: ", error);
            alert("❌ Failed to inject teams. Check console.");
            dummyBtn.innerText = "🛠️ Retry Injection";
...
// Show the button 3 seconds after the app loads to ensure Firebase Auth is ready
setTimeout(injectDatabaseDummyButton, 3000);
